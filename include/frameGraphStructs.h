...
namespace Hydrogen
{
    ...
    struct FGResourceHandle
    {
        ...
        bool IsBuffer() const
        {
            return type == FGResourceType::Buffer;
        }

        FGResourceType GetType() const
        {
            return type;
        }
    ...
    private:
        ...
        uint32 KeyFromName(std::string_view name) const
        {
            ...
            H2_VERIFY_FATAL(m_nameToKey.contains(resourceName), "Resource '{}' was not accessed by any live pass!",
                            name);
