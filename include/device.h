...
namespace Hydrogen
{
    ...
    public:
        ...
        template <eQueueType Queue> uint64 Signal()
        {
            return GetQueue<Queue>().Signal();
        }

        template <eQueueType Queue> void Wait(uint64 fenceValue)
        {
            GetQueue<Queue>().Wait(fenceValue);
        }

        template <eQueueType Queue> void WaitForIdle()
        {
            GetQueue<Queue>().WaitForIdle();
        }

        template <eQueueType Queue> uint64 GetCompletedFenceValue()
        {
            return GetQueue<Queue>().GetCompletedFenceValue();
        }

        template <eQueueType QueueToStall, eQueueType QueueToWaitOn> void WaitOnQueue(uint64 fenceValue)
        {
            GetQueue<QueueToStall>().WaitOnQueue(GetQueue<QueueToWaitOn>(), fenceValue);
        }

        template <eDescriptorHeapType HeapType> const DescriptorHeap& GetDescriptorHeap() const
        {
        ...
        template <eQueueType Queue> ID3D12CommandQueue* GetDxQueue()
        {
        ...
        ID3D12Device14* GetDxDevice() const
        {
            return m_pDxDevice.Get();
        }
        ...
        std::unique_ptr<Texture> CreateTexture(std::wstring_view name, const Texture::Desc& desc,
                                               ResourceState initialState = {},
                                               const D3D12_CLEAR_VALUE* pClearValue = nullptr);
        std::unique_ptr<Texture> CreateTexture(std::wstring_view name, ID3D12Resource* pResource,
                                               const Texture::Desc& desc, ResourceState initialState = {},
                                               const D3D12_CLEAR_VALUE* pClearValue = nullptr);

        std::unique_ptr<Buffer> CreateBuffer(std::wstring_view name, const Buffer::Desc& desc,
                                             ResourceState initialState = {});
        std::unique_ptr<Buffer> CreateBuffer(std::wstring_view name, ID3D12Resource* pResource,
        ...
        RenderTargetViewHandle CreateRenderTargetViewAtIndex(const Texture* pTexture,
                                                             const D3D12_RENDER_TARGET_VIEW_DESC& rtvDesc,
                                                             uint32 rtvIndex);
        DepthStencilViewHandle CreateDepthStencilViewAtIndex(const Texture* pTexture,
                                                             const D3D12_DEPTH_STENCIL_VIEW_DESC& dsvDesc,
                                                             uint32 dsvIndex);
        ShaderResourceViewHandle CreateShaderResourceView(const Buffer* pBuffer,
                                                          const D3D12_SHADER_RESOURCE_VIEW_DESC& srvDesc);
        ShaderResourceViewHandle CreateShaderResourceView(const Texture* pTexture,
                                                          const D3D12_SHADER_RESOURCE_VIEW_DESC& srvDesc);
        void UpdateShaderResourceView(ShaderResourceViewHandle handle, const D3D12_SHADER_RESOURCE_VIEW_DESC& srvDesc);

        UnorderedAccessViewHandle CreateUnorderedAccessView(const Texture* pTexture,
                                                            const D3D12_UNORDERED_ACCESS_VIEW_DESC& uavDesc);
        void UpdateUnorderedAccessView(UnorderedAccessViewHandle handle, ID3D12Resource* pResource,
                                       const D3D12_UNORDERED_ACCESS_VIEW_DESC& uavDesc);

        RenderTargetViewHandle GetRenderTargetHandle(uint32 index) const
        {
            return RenderTargetViewHandle{.dxCpuHandle = m_rtvDescriptorHeap.GetCpuHandle(index)};
        }
        DepthStencilViewHandle GetDepthStencilHandle(uint32 index) const
        {
            return DepthStencilViewHandle{.dxCpuHandle = m_dsvDescriptorHeap.GetCpuHandle(index)};
        }
    ...
    private:
        ...
        template <eQueueType Queue> CommandQueue& GetQueue()
        {
