...
namespace Hydrogen
{
    ...
    {
    public:
        void Initialize(GpuDevice& device, GpuUploader& uploader, uint32 maxVertices, uint32 maxIndices,
                        uint32 sceneCapacity = 100'000, uint32 maxViews = 16, uint32 maxLights = 1024);

        // Enqueues a mesh for upload. Actual GPU upload happens during Update(), up to m_maxMeshUploadsPerFrame per
        // frame.
        ...
        uint32 GetPositionBufferIndex() const
        {
            return m_positionSrv.index;
        }
        uint32 GetNormalBufferIndex() const
        {
        ...
        uint32 GetIndexBufferIndex() const
        {
            return m_indexSrv.index;
        }
        uint32 GetMeshDataBufferIndex() const
        {
            return m_meshDataSrvs[m_currentFrameIndex].index;
        }
        uint32 GetInstanceDataBufferIndex() const
        {
            return m_instanceDataSrvs[m_currentFrameIndex].index;
        }
        uint32 GetTransformBufferIndex() const
        {
            return m_transformSrvs[m_currentFrameIndex].index;
        }
        uint32 GetMaterialDataBufferIndex() const
        {
            return m_materialDataSrvs[m_currentFrameIndex].index;
        }
        uint32 GetLightBufferIndex() const
        {
            return m_lightSrvs[m_currentFrameIndex].index;
        }
        ...
        {
            return m_instanceDescs[m_currentFrameIndex].get();
        }
    ...
    private:
        ...
        struct InFlightMeshUploadData
        {
            MeshHandle handle{};
            uint64 copyFence = 0;
        };
        struct InFlightBlasBuildData
        {
            MeshHandle handle{};
            uint32 blasBufferIndex = 0;
            uint64 buildFence = 0;
        };
