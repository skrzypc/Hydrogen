...
namespace Hydrogen
{
    void CommandQueue::Initialize(ID3D12Device* pDevice, D3D12_COMMAND_LIST_TYPE type)
    {
        ...
        D3D12_COMMAND_QUEUE_DESC commandQueueDesc{.Type = type,
                                                  .Priority = D3D12_COMMAND_QUEUE_PRIORITY_NORMAL,
                                                  .Flags = D3D12_COMMAND_QUEUE_FLAG_NONE,
                                                  .NodeMask = 0u};

        H2_VERIFY_FATAL(pDevice->CreateCommandQueue(&commandQueueDesc, IID_PPV_ARGS(&m_pQueue)),
                        "Failed to create command queue!");
        m_pQueue->SetName(String::Format(L"H2_{}_COMMAND_QUEUE", queueTypeString).c_str());

        H2_VERIFY_FATAL(pDevice->CreateFence(m_fenceValue, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_pFence)),
                        "Failed to create fence!");
        m_pFence->SetName(String::Format(L"H2_{}_FENCE", queueTypeString).c_str());

        m_fenceEvent =
            CreateEventW(nullptr, FALSE, FALSE, String::Format(L"H2_{}_FENCE_EVENT", queueTypeString).c_str());
