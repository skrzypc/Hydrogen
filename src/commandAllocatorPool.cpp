...
namespace Hydrogen
{
    void CommandAllocatorPool::Initialize(GpuDevice& device, D3D12_COMMAND_LIST_TYPE type)
    {
        ...
        {
            H2_VERIFY_FATAL(m_pDevice->GetDxDevice()->CreateCommandAllocator(m_type, IID_PPV_ARGS(&pAllocator)),
    ...
    ID3D12CommandAllocator* CommandAllocatorPool::Acquire(uint64 completedFenceValue)
    {
        ...
        H2_VERIFY_FATAL(m_pDevice->GetDxDevice()->CreateCommandAllocator(m_type, IID_PPV_ARGS(&pNew)),
                        "Failed to create command allocator!");
        return pNew.Get();
    }
