...
namespace Hydrogen
{
    void GpuDevice::Create()
    {
        ...
        if (H2_VERIFY_FATAL(D3D12GetDebugInterface(IID_PPV_ARGS(&pDebugInterface)),
                            "Unable to obtain DX12 debug interface!"))
        {
        ...
        if (H2_VERIFY_FATAL(DXGIGetDebugInterface1(0, IID_PPV_ARGS(&pDXGIInforQueue)),
                            "Unable to obtain DXGI debug interface!"))
        {
        ...
        H2_VERIFY_FATAL(CreateDXGIFactory2(dxgiFactoryFlags, IID_PPV_ARGS(&m_pDxgiFactory)),
        ...
        for (uint32 i = 0;
             DXGI_ERROR_NOT_FOUND != m_pDxgiFactory->EnumAdapterByGpuPreference(i, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE,
                                                                                IID_PPV_ARGS(&m_pDxgiAdapter));
             ++i)
        {
        ...
        H2_VERIFY_FATAL(
            ...
            "Requested adapter could not be found! Perhaps requested adapter vendor is not present on this system.");

        H2_VERIFY_FATAL(D3D12CreateDevice(m_pDxgiAdapter.Get(), D3D_FEATURE_LEVEL_12_1, IID_PPV_ARGS(&m_pDxDevice)),
                        "DX12 device could not be created!");
    ...
    std::unique_ptr<Texture> GpuDevice::CreateTexture(std::wstring_view name, const Texture::Desc& desc,
                                                      ResourceState initialState, const D3D12_CLEAR_VALUE* pClearValue)
    {
        ...
        H2_VERIFY(m_pDxDevice->CreateCommittedResource3(&heapProps, D3D12_HEAP_FLAG_NONE, &resourceDesc,
                                                        initialState.layout, pClearValue, nullptr, 0, nullptr,
    ...
    std::unique_ptr<Texture> GpuDevice::CreateTexture(std::wstring_view name, ID3D12Resource* pResource,
                                                      const Texture::Desc& desc, ResourceState initialState,
    ...
    std::unique_ptr<Buffer> GpuDevice::CreateBuffer(std::wstring_view name, const Buffer::Desc& desc,
                                                    ResourceState initialState)
    {
        ...
        H2_VERIFY(m_pDxDevice->CreateCommittedResource3(&heapProps, D3D12_HEAP_FLAG_NONE, &resourceDesc,
                                                        initialState.layout, nullptr, nullptr, 0, nullptr,
    ...
    std::unique_ptr<Buffer> GpuDevice::CreateBuffer(std::wstring_view name, ID3D12Resource* pResource,
                                                    const Buffer::Desc& desc, ResourceState initialState)
    {
        auto pBuffer = std::make_unique<Buffer>();
    ...
    std::unique_ptr<UploadBuffer> GpuDevice::CreateUploadBuffer(std::wstring_view name, uint64 sizeInBytes)
    {
        ...
        H2_VERIFY_FATAL(m_pDxDevice->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &resourceDesc,
        ...
        H2_VERIFY_FATAL(pBuffer->GetResource()->Map(0, nullptr, reinterpret_cast<void**>(&pMapped)),
    ...
    RenderTargetViewHandle GpuDevice::CreateRenderTargetViewAtIndex(const Texture* pTexture,
    ...
    DepthStencilViewHandle GpuDevice::CreateDepthStencilViewAtIndex(const Texture* pTexture,
    ...
    ShaderResourceViewHandle GpuDevice::CreateShaderResourceView(const Buffer* pBuffer,
    ...
    ShaderResourceViewHandle GpuDevice::CreateShaderResourceView(const Texture* pTexture,
                                                                 const D3D12_SHADER_RESOURCE_VIEW_DESC& srvDesc)
    {
        uint32 index = m_cbvSrvUavDescriptorHeap.Allocate(1);
    ...
    void GpuDevice::UpdateShaderResourceView(ShaderResourceViewHandle handle,
                                             const D3D12_SHADER_RESOURCE_VIEW_DESC& srvDesc)
    {
        m_pDxDevice->CreateShaderResourceView(nullptr, &srvDesc, m_cbvSrvUavDescriptorHeap.GetCpuHandle(handle.index));
    }

    UnorderedAccessViewHandle GpuDevice::CreateUnorderedAccessView(const Texture* pTexture,
                                                                   const D3D12_UNORDERED_ACCESS_VIEW_DESC& uavDesc)
    {
        uint32 index = m_cbvSrvUavDescriptorHeap.Allocate(1);
        ID3D12Resource* pResource = pTexture ? pTexture->GetResource() : nullptr;
        m_pDxDevice->CreateUnorderedAccessView(pResource, nullptr, &uavDesc,
                                               m_cbvSrvUavDescriptorHeap.GetCpuHandle(index));
        return UnorderedAccessViewHandle{.index = index};
    }

    void GpuDevice::UpdateUnorderedAccessView(UnorderedAccessViewHandle handle, ID3D12Resource* pResource,
                                              const D3D12_UNORDERED_ACCESS_VIEW_DESC& uavDesc)
    {
        m_pDxDevice->CreateUnorderedAccessView(pResource, nullptr, &uavDesc,
                                               m_cbvSrvUavDescriptorHeap.GetCpuHandle(handle.index));
    }

    void GpuDevice::Initialize()
    {
        m_directCommandQueue.Initialize(GetDxDevice(), D3D12_COMMAND_LIST_TYPE_DIRECT);

        m_copyCommandQueue.Initialize(GetDxDevice(), D3D12_COMMAND_LIST_TYPE_COPY);
        ...
        m_cbvSrvUavDescriptorHeap.Initialize(GetDxDevice(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 16384,
                                             L"H2_CBV_SRV_UAV_DESCRIPTOR_HEAP");
        m_cbvSrvUavDescriptorAllocator =
            RequestDescriptorAllocator<LinearIndexAllocator>(8192, eDescriptorHeapType::CBV_SRV_UAV);

        m_samplerDescriptorHeap.Initialize(GetDxDevice(), D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, 2048,
                                           L"H2_SAMPLER_DESCRIPTOR_HEAP");
        m_samplerDescriptorAllocator =
            RequestDescriptorAllocator<LinearIndexAllocator>(1024, eDescriptorHeapType::Sampler);

        m_rtvDescriptorHeap.Initialize(GetDxDevice(), D3D12_DESCRIPTOR_HEAP_TYPE_RTV, 1024, L"H2_RTV_DESCRIPTOR_HEAP");
        m_rtvDescriptorAllocator = RequestDescriptorAllocator<LinearIndexAllocator>(512, eDescriptorHeapType::RTV);

        m_dsvDescriptorHeap.Initialize(GetDxDevice(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV, 1024, L"H2_DSV_DESCRIPTOR_HEAP");
    ...
    bool GpuDevice::CheckRequiredFeatureSupport() const
    {
        ...
        // Shader model
        {
            D3D_SHADER_MODEL expectedShaderModel =
                static_cast<D3D_SHADER_MODEL>((Config::ShaderModelVersionMajor << 4) | Config::ShaderModelVersionMinor);

            D3D12_FEATURE_DATA_SHADER_MODEL featureData{expectedShaderModel};
            H2_VERIFY(m_pDxDevice->CheckFeatureSupport(D3D12_FEATURE_SHADER_MODEL, &featureData, sizeof(featureData)),
                      "Could not check shader model support!");
        ...
        // Raytracing
        {
            ...
            H2_VERIFY(m_pDxDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS5, &featureData, sizeof(featureData)),
                      "Could not check raytracing support!");
        ...
        // Mesh shaders
        {
            ...
            H2_VERIFY(m_pDxDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS7, &featureData, sizeof(featureData)),
                      "Could not check mesh shader support!");
        ...
        // Bindless resources
        {
            ...
            H2_VERIFY(m_pDxDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS, &featureData, sizeof(featureData)),
                      "Could not check bindless resource support!");
        ...
        // Enhanced barriers
        {
            ...
            H2_VERIFY(
                m_pDxDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS12, &featureData, sizeof(featureData)),
                "Could not check enhanced barriers support!");
    ...
    GraphicsContext GpuDevice::AcquireGraphicsContext()
    {
        ...
        // H2_VERIFY_FATAL(GraphicsContext::s_frameDataAddr != 0, "Frame data address not set before acquiring graphics
        // context!");
        pList->SetGraphicsRootConstantBufferView(static_cast<uint32>(eRootParam::FrameConstantBuffer),
