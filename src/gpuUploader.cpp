...
namespace Hydrogen
{
    ...
    uint64 GpuUploader::Allocate(uint64 byteSize)
    {
        ...
        for (int attempt = 0; attempt < 2; ++attempt)
        {
            ...
            {
                const bool blocked = !m_pendingSegments.empty() && m_pendingSegments.front().start >= m_writeOffset &&
    ...
    void GpuUploader::Upload(const void* pData, uint64 byteSize, Buffer* pDstBuffer, uint64 dstOffset)
    {
        ...
        m_activeContext->CmdList()->CopyBufferRegion(pDstBuffer->GetResource(), dstOffset,
                                                     m_stagingBuffer->GetResource(), stagingOffset, byteSize);
