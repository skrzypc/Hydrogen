...
namespace Hydrogen
{
    template <D3D12_PIPELINE_STATE_SUBOBJECT_TYPE Type, typename T> struct alignas(void*) PSOSubObject
    {
    ...
    struct GraphicsPipelineStream
    {
        ...
        PSOSubObject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PRIMITIVE_TOPOLOGY, D3D12_PRIMITIVE_TOPOLOGY_TYPE>
            primitiveTopology;
    };
    ...
    void PipelineState::CreateGraphics(GpuDevice& device, const GraphicsDesc& desc)
    {
        ...
        H2_VERIFY_FATAL(!desc.pPixelShader || desc.pPixelShader->GetDesc().type == eShaderType::PS,
                        "Expected a pixel shader!");
        ...
        stream.ps.data = desc.pPixelShader ? D3D12_SHADER_BYTECODE{desc.pPixelShader->GetBytecode(),
                                                                   desc.pPixelShader->GetBytecodeSize()}
        ...
        H2_VERIFY_FATAL(device.GetDxDevice()->CreatePipelineState(&streamDesc, IID_PPV_ARGS(&m_pDxPipelineState)),
