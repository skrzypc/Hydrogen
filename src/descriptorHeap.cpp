...
namespace Hydrogen
{
    void DescriptorHeap::Initialize(ID3D12Device* pDevice, D3D12_DESCRIPTOR_HEAP_TYPE type, uint32 capacity,
                                    std::wstring_view name)
    {
        ...
        H2_VERIFY_FATAL(pDevice->CreateDescriptorHeap(&descriptorHeapDesc, IID_PPV_ARGS(&m_pDxDescriptorHeap)),
                        "Failed to create descriptor heap!");
        m_pDxDescriptorHeap->SetName(name.data());

        m_cpuHandleStart = m_pDxDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
        m_gpuHandleStart = bShaderVisible ? m_pDxDescriptorHeap->GetGPUDescriptorHandleForHeapStart()
    ...
    {
        if (!H2_VERIFY(m_gpuHandleStart.ptr != std::numeric_limits<uint64>::max(),
                       "Attempted to access a GPU descriptor from a non-shader-visible descriptor heap!"))
        {
