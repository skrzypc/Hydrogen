...
namespace Hydrogen
{
    ShaderCompiler::ShaderCompiler()
    {
        ...
        {
            H2_VERIFY_FATAL(std::filesystem::create_directories(m_shaderOutputDir),
                            "Failed to create shader output directory: {}", m_shaderOutputDir.string());
        }

        // Create DXC objects
        H2_VERIFY_FATAL(DxcCreateInstance(CLSID_DxcUtils, IID_PPV_ARGS(&m_pUtils)), "Failed to create IDxcUtils");
        H2_VERIFY_FATAL(DxcCreateInstance(CLSID_DxcCompiler, IID_PPV_ARGS(&m_pCompiler)),
                        "Failed to create IDxcCompiler3");
        H2_VERIFY_FATAL(m_pUtils->CreateDefaultIncludeHandler(&m_pIncludeHandler),
                        "Failed to create DXC include handler");

        const std::wstring smSuffix =
            String::Format(L"_{}_{}", Config::ShaderModelVersionMajor, Config::ShaderModelVersionMinor);
    ...
    bool ShaderCompiler::Compile(Shader& shader)
    {
        ...
        H2_VERIFY_FATAL(std::filesystem::exists(fullSourcePath), "Shader source not found: {}",
                        fullSourcePath.string());

        Microsoft::WRL::ComPtr<IDxcBlobEncoding> pSource = nullptr;
        H2_VERIFY_FATAL(m_pUtils->LoadFile(fullSourcePath.c_str(), nullptr, &pSource), "Failed to load shader: {}",
                        fullSourcePath.string());
        ...
        auto addArg = [&](std::wstring s) { argPtrs.push_back(argStorage.emplace_back(std::move(s)).c_str()); };
        ...
        {
            addArg(L"-E");
            addArg(String::ToWide(desc.entryPoint));
        }

        // Target profile
        addArg(L"-T");
        addArg(m_targetStrings.at(desc.type));
        ...
        // Include paths
        addArg(L"-I");
        addArg((m_shaderSourceDir).wstring());
        addArg((m_shaderSourceDir / "include").wstring());
        ...
        H2_VERIFY_FATAL(m_pCompiler->Compile(&sourceBuffer, argPtrs.data(), static_cast<uint32>(argPtrs.size()),
                                             m_pIncludeHandler.Get(), IID_PPV_ARGS(&pResult)),
        ...
        if (FAILED(status))
        {
            ...
            if (SUCCEEDED(pResult->GetOutput(DXC_OUT_ERRORS, IID_PPV_ARGS(&pErrors), nullptr)) && pErrors &&
        ...
        // Retrieve binary.
        {
            ...
            H2_VERIFY_FATAL(pResult->GetOutput(DXC_OUT_OBJECT, IID_PPV_ARGS(&shader.m_pBinary), &pOutName),
            ...
            if (pOutName && pOutName->GetStringLength() > 0)
            {
                ...
                if (file)
                {
                    file.write(static_cast<const char*>(shader.m_pBinary->GetBufferPointer()),
        ...
        // Retrieve PDB.
        {
            ...
            if (SUCCEEDED(pResult->GetOutput(DXC_OUT_PDB, IID_PPV_ARGS(&pPdb), &pPdbName)) && pPdb && pPdbName &&
                pPdbName->GetStringLength() > 0)
            {
                std::ofstream file(pPdbName->GetStringPointer(), std::ios::out | std::ios::binary);
                if (file)
                {
                    file.write(static_cast<const char*>(pPdb->GetBufferPointer()),
        ...
        // Hash.
        {
            ...
            if (H2_VERIFY(pResult->GetOutput(DXC_OUT_SHADER_HASH, IID_PPV_ARGS(&pHashBlob), nullptr),
                          "Can't obtain shader hash!") &&
                pHashBlob)
            {
