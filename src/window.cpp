...
namespace Hydrogen
{
    ...
    void Window::Create(const uint32 width, const uint32 height, const std::wstring_view& windowTitle,
                        const std::wstring_view& windowClassName)
    {
        ...
        const WNDCLASSEX wndClass{.cbSize = sizeof(WNDCLASSEX),
                                  .style = CS_OWNDC,
        ...
        m_hwnd = CreateWindowEx(0, windowClassName.data(), windowTitle.data(), windowStyle, 100, 40,
                                adjustedWndSize.right - adjustedWndSize.left,
                                adjustedWndSize.bottom - adjustedWndSize.top, nullptr, nullptr, m_hInstance, this);
    ...
    LRESULT Window::WindowProcedure(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        case WM_MOUSEWHEEL:
        {
            m_currentInput.scrollDelta +=
                static_cast<float32>(GET_WHEEL_DELTA_WPARAM(wParam)) / static_cast<float32>(WHEEL_DELTA);
            break;
        }
        case WM_XBUTTONDOWN:
        case WM_XBUTTONUP:
    ...
    bool Window::IsKeyDown(uint8 keyCode) const
    {
        return m_currentInput.keys[keyCode];
    }
    bool Window::IsKeyJustPressed(uint8 keyCode) const
    {
        return m_currentInput.keys[keyCode] && !m_previousInput.keys[keyCode];
    }
    bool Window::IsKeyJustReleased(uint8 keyCode) const
    {
        return !m_currentInput.keys[keyCode] && m_previousInput.keys[keyCode];
    }
    ...
    {
        return m_currentInput.leftMouseDown;
    }
    bool Window::IsLeftMouseJustPressed() const
    {
        return m_currentInput.leftMouseDown && !m_previousInput.leftMouseDown;
    }
    bool Window::IsLeftMouseJustReleased() const
    {
        return !m_currentInput.leftMouseDown && m_previousInput.leftMouseDown;
    }

    bool Window::IsMiddleMouseDown() const
    {
        return m_currentInput.middleMouseDown;
    }
    bool Window::IsMiddleMouseJustPressed() const
    {
        return m_currentInput.middleMouseDown && !m_previousInput.middleMouseDown;
    }
    bool Window::IsMiddleMouseJustReleased() const
    {
        return !m_currentInput.middleMouseDown && m_previousInput.middleMouseDown;
    }
    ...
    {
        return m_currentInput.rightMouseDown;
    }
    bool Window::IsRightMouseJustPressed() const
    {
        return m_currentInput.rightMouseDown && !m_previousInput.rightMouseDown;
    }
    bool Window::IsRightMouseJustReleased() const
    {
        return !m_currentInput.rightMouseDown && m_previousInput.rightMouseDown;
    }

    float32 Window::GetMouseDeltaX() const
    {
        return m_currentInput.mouseDeltaX;
    }
    float32 Window::GetMouseDeltaY() const
    {
        return m_currentInput.mouseDeltaY;
    }
    float32 Window::GetMousePosX() const
    {
        return m_currentInput.mousePosX;
    }
    float32 Window::GetMousePosY() const
    {
        return m_currentInput.mousePosY;
    }
    float32 Window::GetScrollDelta() const
    {
        return m_currentInput.scrollDelta;
    }
