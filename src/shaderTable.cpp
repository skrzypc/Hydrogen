...
namespace Hydrogen
{
    namespace
    {
        ...
        constexpr uint32 RecordStride =
            static_cast<uint32>(AlignUp(ShaderIdentifierSize, D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT));
    } // namespace
    ...
    void ShaderTable::Create(GpuDevice& device, const RaytracingPipelineState& pso, const Desc& desc)
    {
        ...
        m_missTableOffset =
            AlignUp(m_rayGenTableOffset + rayGenTableSize, D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT);
        const uint64 missTableSize = static_cast<uint64>(m_missCount) * RecordStride;

        m_hitGroupTableOffset =
            AlignUp(m_missTableOffset + missTableSize, D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT);
    ...
    D3D12_DISPATCH_RAYS_DESC ShaderTable::GetDispatchRaysDesc(uint32 rayGenIndex, uint32 width, uint32 height) const
    {
        ...
        dispatchDesc.RayGenerationShaderRecord.StartAddress =
            base + m_rayGenTableOffset + static_cast<uint64>(rayGenIndex) * m_recordStride;
