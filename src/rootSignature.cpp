...
namespace Hydrogen
{
    void RootSignature::Create(GpuDevice& device)
    {
            ...
            .Descriptor =
                {
                    .ShaderRegister = 1,
                    .RegisterSpace = 0,
                    .Flags = D3D12_ROOT_DESCRIPTOR_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE, // Stable between binding and
                                                                                          // the draw/dispatch.
                },
            .ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL,
        };
            ...
            .Descriptor =
                {
                    .ShaderRegister = 2,
                    .RegisterSpace = 0,
                    .Flags = D3D12_ROOT_DESCRIPTOR_FLAG_DATA_STATIC, // Guaranteed not to change for the lifetime of the
                                                                     // command list.
                },
            .ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL,
        };
            ...
            .Desc_1_2 =
                {
                    ...
                    .Flags =
                        D3D12_ROOT_SIGNATURE_FLAG_CBV_SRV_UAV_HEAP_DIRECTLY_INDEXED | // enables
                                                                                      // ResourceDescriptorHeap[i] in
                                                                                      // HLSL
                        D3D12_ROOT_SIGNATURE_FLAG_SAMPLER_HEAP_DIRECTLY_INDEXED | // enables SamplerDescriptorHeap[i] in
                                                                                  // HLSL
        ...
        H2_VERIFY_FATAL(D3D12SerializeVersionedRootSignature(&rsDesc, &pBlob, &pErrorBlob),
                        "Root signature serialization failed: {}",
                        pErrorBlob ? static_cast<const char*>(pErrorBlob->GetBufferPointer()) : "Unknown error.");

        H2_VERIFY_FATAL(device.GetDxDevice()->CreateRootSignature(0, pBlob->GetBufferPointer(), pBlob->GetBufferSize(),
    ...
    std::array<D3D12_STATIC_SAMPLER_DESC1, static_cast<size_t>(eStaticSampler::Count)> RootSignature::
        BuildStaticSamplers()
    {
        ...
        staticSamplers[static_cast<uint32>(eStaticSampler::LinearWrap)].ShaderRegister =
            static_cast<UINT>(eStaticSampler::LinearWrap);

        // s1 — linear clamp.
        staticSamplers[static_cast<uint32>(eStaticSampler::LinearClamp)] =
            staticSamplers[static_cast<uint32>(eStaticSampler::LinearWrap)];
        ...
        staticSamplers[static_cast<uint32>(eStaticSampler::LinearClamp)].ShaderRegister =
            static_cast<UINT>(eStaticSampler::LinearClamp);

        // s2 — point clamp (depth reads, ID buffers, exact texel fetch).
        staticSamplers[static_cast<uint32>(eStaticSampler::PointClamp)] =
            staticSamplers[static_cast<uint32>(eStaticSampler::LinearClamp)];
        staticSamplers[static_cast<uint32>(eStaticSampler::PointClamp)].Filter = D3D12_FILTER_MIN_MAG_MIP_POINT;
        staticSamplers[static_cast<uint32>(eStaticSampler::PointClamp)].ShaderRegister =
            static_cast<UINT>(eStaticSampler::PointClamp);

        // s3 — anisotropic 16x wrap (high-quality surface textures).
        staticSamplers[static_cast<uint32>(eStaticSampler::AnisoWrap)] =
            staticSamplers[static_cast<uint32>(eStaticSampler::LinearWrap)];
        ...
        staticSamplers[static_cast<uint32>(eStaticSampler::AnisoWrap)].ShaderRegister =
            static_cast<UINT>(eStaticSampler::AnisoWrap);

        // s4 — linear border, opaque black (shadow map out-of-bounds = fully lit), no mip interpolation on shadow maps.
        staticSamplers[static_cast<uint32>(eStaticSampler::LinearBorder)] =
            staticSamplers[static_cast<uint32>(eStaticSampler::LinearClamp)];
        staticSamplers[static_cast<uint32>(eStaticSampler::LinearBorder)].Filter =
        ...
        staticSamplers[static_cast<uint32>(eStaticSampler::LinearBorder)].BorderColor =
            D3D12_STATIC_BORDER_COLOR_OPAQUE_BLACK;
        staticSamplers[static_cast<uint32>(eStaticSampler::LinearBorder)].ShaderRegister =
            static_cast<UINT>(eStaticSampler::LinearBorder);
