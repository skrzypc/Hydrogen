...
namespace Hydrogen
{
    void FGResourceCache::Initialize(GpuDevice& device, uint32 rtvCapacity, uint32 dsvCapacity)
    {
        m_pDevice = &device;
        m_rtvAllocator =
            m_pDevice->RequestDescriptorAllocator<FreeListIndexAllocator>(rtvCapacity, eDescriptorHeapType::RTV);
        m_dsvAllocator =
            m_pDevice->RequestDescriptorAllocator<FreeListIndexAllocator>(dsvCapacity, eDescriptorHeapType::DSV);
    }
    ...
    ShaderResourceViewHandle FGResourceCache::GetAccelerationStructureSRV(const Buffer* pBuffer)
    {
        ...
        ShaderResourceViewHandle srvHandle =
            m_pDevice->CreateShaderResourceView(static_cast<const Buffer*>(nullptr), desc);
    ...
    {
        uint64 seed = 0;
        auto combine = [&](auto v) { seed ^= std::hash<decltype(v)>{}(v) + 0x9e3779b9u + (seed << 6) + (seed >> 2); };
