...
namespace Hydrogen
{
    void RaytracingPipelineState::Create(GpuDevice& device, const Desc& desc)
    {
        ...
        H2_VERIFY_FATAL(device.GetDxDevice()->CreateStateObject(&stateObjectDesc, IID_PPV_ARGS(&m_pDxStateObject)),
                        "Raytracing state object creation failed!");
        H2_VERIFY_FATAL(m_pDxStateObject.As(&m_pDxStateObjectProperties),
                        "Failed to query ID3D12StateObjectProperties!");
    }
