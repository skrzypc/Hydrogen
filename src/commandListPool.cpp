...
namespace Hydrogen
{
    ...
    ID3D12GraphicsCommandList10* CommandListPool::Acquire()
    {
        ...
        H2_VERIFY_FATAL(m_pDevice->GetDxDevice()->CreateCommandAllocator(m_type, IID_PPV_ARGS(&pTempAllocator)),
                        "Failed to create temp allocator!");

        auto& pNew = m_allLists.emplace_back();
        H2_VERIFY_FATAL(
            m_pDevice->GetDxDevice()->CreateCommandList(0, m_type, pTempAllocator.Get(), nullptr, IID_PPV_ARGS(&pNew)),
