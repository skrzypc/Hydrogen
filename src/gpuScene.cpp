...
namespace Hydrogen
{
    ...
    void GpuScene::Initialize(GpuDevice& device, GpuUploader& uploader, uint32 maxVertices, uint32 maxIndices,
                              uint32 sceneCapacity, uint32 maxViews, uint32 maxLights)
    {
        m_pDevice = &device;
        ...
        m_positionBuffer = device.CreateBuffer(
        ...
        m_uvBuffer = device.CreateBuffer(L"H2_SCENE_UVS", Buffer::Desc{.size = maxVertices * sizeof(DirectX::XMFLOAT2)},
                                         initialState);
        m_indexBuffer =
            device.CreateBuffer(L"H2_SCENE_INDICES", Buffer::Desc{.size = maxIndices * sizeof(uint32)}, initialState);
        ...
        {
            m_transformBuffers[i] =
                device.CreateUploadBuffer(L"H2_SCENE_TRANSFORMS", m_sceneCapacity * sizeof(DirectX::XMFLOAT4X4));
            ...
            m_meshDataBuffers[i] =
                device.CreateUploadBuffer(L"H2_SCENE_MESH_DATA", m_sceneCapacity * sizeof(GpuMeshData));
            ...
            m_instanceDataBuffers[i] =
                device.CreateUploadBuffer(L"H2_SCENE_INSTANCE_DATA", m_sceneCapacity * sizeof(GpuInstanceData));
            ...
            m_materialDataBuffers[i] =
                device.CreateUploadBuffer(L"H2_SCENE_MATERIAL_DATA", m_sceneCapacity * sizeof(GpuMaterialData));
            ...
            m_instanceDescs[i] = device.CreateUploadBuffer(L"H2_SCENE_TLAS_INSTANCES",
                                                           m_sceneCapacity * sizeof(D3D12_RAYTRACING_INSTANCE_DESC));
    ...
    void GpuScene::BuildBlas(std::span<const InFlightMeshUploadData> uploads)
    {
        ...
        for (uint32 i = 0; i < uploads.size(); ++i)
        {
            ...
            rtGeometryDescs[i].Triangles.VertexBuffer.StartAddress =
                m_positionBuffer->GetResource()->GetGPUVirtualAddress() + mesh.baseVertex * sizeof(DirectX::XMFLOAT3);
            ...
            rtGeometryDescs[i].Triangles.IndexBuffer =
                m_indexBuffer->GetResource()->GetGPUVirtualAddress() + mesh.baseIndex * sizeof(uint32);
            ...
            const uint64 alignedScratch =
                (blasPrebuildInfos[i].ScratchDataSizeInBytes + D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT -
                 1) &
        ...
        if (!scratch || scratch->GetDesc().size < totalScratchSize)
        {
            ...
            scratch = m_pDevice->CreateBuffer(
                L"H2_BLAS_SCRATCH",
                Buffer::Desc{.size = totalScratchSize, .flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS}, uavState);
        }
        ...
        for (uint32 i = 0; i < uploads.size(); ++i)
        {
            ...
            m_blasBuffers.push_back(
                m_pDevice->CreateBuffer(String::ToWide(String::Format("H2_BLAS_{}", String::ToUpper(mesh.name))),
                                        Buffer::Desc{.size = blasPrebuildInfos[i].ResultDataMaxSizeInBytes,
    ...
    void GpuScene::UpdateMeshData()
    {
        ...
        {
            m_meshDataBuffers[m_currentFrameIndex]->Write(m_meshDataStaging.data(),
    ...
    void GpuScene::UpdateTransforms(std::span<const RenderObject> objects)
    {
        ...
        auto* pDescs =
            reinterpret_cast<D3D12_RAYTRACING_INSTANCE_DESC*>(m_instanceDescs[m_currentFrameIndex]->GetMappedPtr());
        ...
        {
            m_transformBuffers[m_currentFrameIndex]->Write(m_transformStaging.data(),
                                                           m_transformStaging.size() * sizeof(DirectX::XMFLOAT4X4));
            m_instanceDataBuffers[m_currentFrameIndex]->Write(m_instanceDataStaging.data(),
    ...
    void GpuScene::UploadMeshGeometry(MeshHandle handle, const Mesh& src)
    {
        ...
        m_pUploader->Upload(src.positions.data(), vertexCount * sizeof(DirectX::XMFLOAT3), m_positionBuffer.get(),
